...
2. МЕСТО РАБОТЫ, ФОРМАТ И ЛОКАЦИЯ:
   ...
   - Если вакансия требует строго очного присутствия в офисе в городе, где кандидат не живет, и в резюме не указана готовность к переезду в этот город — снижайте оценку и устанавливайте `is_match = False`.
   - Если вакансия удаленная или находится в городе проживания кандидата — формат считается подходящим.
...
def init_db():
    ...
    if current_prompt_row:
        ...
        if "РЕЛЕВАНТНЫЙ коммерческий опыт" not in current_prompt:
